    ...
    function updateDecorations() {
        ...
        for (let i = 0; i < extSections.length; i++) {
            ...
            extensionSections.push({
                range: new Range(
                    activeEditor.document.positionAt(sectionPosStart),
                    activeEditor.document.positionAt(sectionPosEnd)
                ),
            });
        }
        ...
        middleware: {
            provideCompletionItem: async (document, position, context, token, next) => {
                ...
                virtualDocumentContents.set(
                    originalUri,
                    getVirtualContent(region, document.getText())
                );

                const vdocUriString = `embedded-content://${region}/${encodeURIComponent(
                    originalUri
                )}.${region}`;
